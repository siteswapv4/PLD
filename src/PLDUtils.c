...
PLD_DataIni* PLD_LoadDataIni(const char* path)
{
    ...
    char* word = NULL;
    char* value = NULL;
    SDL_IOStream* iniFile = NULL;
	PLD_DataIni* dataIni = NULL;

	if ((iniFile = SDL_IOFromFile(path, "rb")) == NULL)
	{
		goto error;
	}
	
	dataIni = (PLD_DataIni*)SDL_calloc(1, sizeof(PLD_DataIni));

    for (i = 0; i < 3; i++)
    {
        PLD_ExtractSequence(iniFile, &word, "\n\t\r ", 4);
    }

	stop = PLD_ExtractSequence(iniFile, &word, "=\n\t\r", 4);
    stop = PLD_ExtractSequence(iniFile, &value, "\n\r", 2);

    while (!stop)
    {
        if (SDL_strcmp(word, "start") == 0)
        {
            dataIni->start = SDL_atof(value) * 1000;
        }
        else if (SDL_strcmp(word, "end") == 0)
        {
            dataIni->end = SDL_atof(value) * 1000;
        }
        else if (SDL_strcmp(word, "bpm") == 0)
        {
            dataIni->bpm = SDL_atof(value);
        }
        else if (SDL_strcmp(word, "thumbtimestart") == 0)
        {
        	dataIni->thumbTimeStart = SDL_atof(value) * 1000;
        }
        else if (SDL_strcmp(word, "thumbtimeend") == 0)
        {
        	dataIni->thumbTimeEnd = SDL_atof(value) * 1000;
        }
