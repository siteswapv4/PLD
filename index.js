...
var ASM_CONSTS = {
  8717188: () => { var input = document.createElement('input'); input.type = 'file'; input.webkitdirectory = true; input.multiple = true; input.style.display = 'none'; document.body.appendChild(input); input.onchange = async function(ev) { var files = ev.target.files; if (files.length === 0) { document.body.removeChild(input); return; } var mountPoint = "/picked"; var rootDirName = files[0].webkitRelativePath.split("/")[0]; var fullMountPath = mountPoint + "/" + rootDirName; try { FS.mkdir(mountPoint); } catch(e) {} for (var i = 0; i < files.length; i++) { let file = files[i]; let relPath = file.webkitRelativePath || file.name; let fullPath = mountPoint + "/" + relPath; let parts = relPath.split("/"); let dirPath = mountPoint; for (let j = 0; j < parts.length - 1; j++) { dirPath += "/" + parts[j]; try { FS.mkdir(dirPath); } catch(e) {} } let data = new Uint8Array(await file.arrayBuffer()); FS.writeFile(fullPath, data); } var sp = _malloc(fullMountPath.length + 1); stringToUTF8(fullMountPath, sp, fullMountPath.length + 1); Module.ccall("PLD_CallDirectoryCallback", "void", ["number"], [sp]); _free(sp); document.body.removeChild(input); }; input.click(); },  
 8718343: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return reply.length === 1 ? reply.charCodeAt(0) : -1; },  
 8718558: () => { if (typeof(Module['SDL3']) === 'undefined') { Module['SDL3'] = {}; } Module['SDL3'].dummy_audio = {}; Module['SDL3'].dummy_audio.timers = []; Module['SDL3'].dummy_audio.timers[0] = undefined; Module['SDL3'].dummy_audio.timers[1] = undefined; },  
 8718804: ($0, $1, $2, $3, $4) => { var a = Module['SDL3'].dummy_audio; if (a.timers[$0] !== undefined) { clearInterval(a.timers[$0]); } a.timers[$0] = setInterval(function() { dynCall('vi', $3, [$4]); }, ($1 / $2) * 1000); },  
 8718996: ($0) => { var a = Module['SDL3'].dummy_audio; if (a.timers[$0] !== undefined) { clearInterval(a.timers[$0]); } a.timers[$0] = undefined; },  
 8719127: ($0) => { var parms = new URLSearchParams(window.location.search); for (const [key, value] of parms) { if (key.startsWith("SDL_")) { var ckey = stringToNewUTF8(key); var cvalue = stringToNewUTF8(value); if ((ckey != 0) && (cvalue != 0)) { dynCall('iiii', $0, [ckey, cvalue, 1]); } _free(ckey); _free(cvalue); } } },  
 8719434: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 8719581: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
...
var _PLD_CallDirectoryCallback = Module['_PLD_CallDirectoryCallback'] = makeInvalidEarlyAccess('_PLD_CallDirectoryCallback');
...
function assignWasmExports(wasmExports) {
  ...
  Module['_PLD_CallDirectoryCallback'] = _PLD_CallDirectoryCallback = createExportWrapper('PLD_CallDirectoryCallback', 1);
...
var _xmp_version = Module['_xmp_version'] = 8716880;
var _xmp_vercode = Module['_xmp_vercode'] = 686536;var wasmImports = {
  /** @export */
